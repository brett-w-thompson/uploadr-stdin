...
#
HISTORY_FILE = "uploadr.history.log"
...
class UploadHistory:
	...
	self.data = self.fd.read().split('\n')
#        print self.data
	self.fd.close()

      self.fd = open(filename, 'a+')

    def uploaded(self, image, photo_id=None):
      return image in self.data

    def log_upload(self, image):
      self.fd.write("%s\n" % image)
      self.fd.flush()
...
class Uploadr:
    ...
    def grabNewImages( self ):
        """ grabNewImages
        """
        return natural_sort(sys.stdin.read().split('\n')[:-1])
	...
	self.history.log_upload(imageName)
